...
class ActionModule(ActionBase):
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        if result.get('failed', False) or (result.get('json', {}).get('success', None) is False):
            # Check if the error code is in the ignore list
            ignore_api_errors = task_args.get('ignore_api_errors', [])
            error_code = result.get('json', {}).get('error', {}).get('code', None)
            if ignore_api_errors and error_code in ignore_api_errors:
                result['failed'] = False
                result['ignored_api_error'] = error_code
            else:
                result['failed'] = True
